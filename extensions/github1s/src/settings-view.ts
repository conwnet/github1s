...
export class SettingsView implements vscode.WebviewViewProvider {
				...
				default:
					...
					(oauthToken
						? validateToken(oauthToken).then(
								(data) => data.valid && data.remaining > 0
						  )
	...
	handleValidateToken(token: string) {
		...
		validateToken(token)
			.then((tokenStatus) => {
				if (!tokenStatus.valid) {
					...
					vscode.window.showWarningMessage(
						'This GitHub OAuth Token is valid, but the rate limit is exceeded.'
					);
				} else {
					vscode.window.showInformationMessage(
						'This GitHub OAuth Token is OK.'
					);
				}
				this.updateWebviewState({
			...
			.catch(() =>
				this.updateWebviewState({ valid: false, validating: false })
			);
	}
