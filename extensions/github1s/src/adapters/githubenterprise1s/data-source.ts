...
const sourcegraphDataSource = SourcegraphDataSource.getInstance('github-enterprise');
...
export class GitHubEnterprise1sDataSource extends DataSource {
	private static instance: GitHubEnterprise1sDataSource | null = null;
	...
	public static getInstance(): GitHubEnterprise1sDataSource {
		if (GitHubEnterprise1sDataSource.instance) {
			return GitHubEnterprise1sDataSource.instance;
		}
		return (GitHubEnterprise1sDataSource.instance = new GitHubEnterprise1sDataSource());
	}
	...
	async provideDirectory(repoFullName: string, ref: string, path: string, recursive = false): Promise<Directory> {
		const fetcher = GitHubEnterpriseFetcher.getInstance();
