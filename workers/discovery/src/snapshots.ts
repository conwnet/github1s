...
export const getGitHubHistoryKey = (scheduledTime: number): string =>
	`github:${new Date(scheduledTime).toISOString().slice(0, 10)}`;
...
export const saveSnapshot = async (
	...
	await kv.put(getGitHubHistoryKey(scheduledTime), json, { expiration });
