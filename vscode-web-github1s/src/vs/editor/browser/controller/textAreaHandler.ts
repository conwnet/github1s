...
export class TextAreaHandler extends ViewPart {
	...
	private _setAccessibilityOptions(options: IComputedEditorOptions): void {
		...
		if (this._accessibilitySupport === AccessibilitySupport.Enabled && accessibilityPageSize === EditorOptions.accessibilityPageSize.defaultValue) {
			// If a screen reader is attached and the default value is not set we should automatically increase the page size to 100 for a better experience
	...
	public onConfigurationChanged(e: viewEvents.ViewConfigurationChangedEvent): boolean {
		...
		// only set readonly attribute to textarea when use a mobile device
		// https://github.com/microsoft/vscode/issues/117755
		if (platform.isMobile && e.hasChanged(EditorOption.readOnly)) {
