...
import {
	ITitleService,
	ITitleProperties,
} from 'vs/workbench/services/title/common/titleService';
...
export class TitlebarPart extends Part implements ITitleService {
	private static readonly NLS_UNSUPPORTED = nls.localize(
		'patchedWindowTitle',
		'[Unsupported]'
	);
	private static readonly NLS_USER_IS_ADMIN = isWindows
		? nls.localize('userIsAdmin', '[Administrator]')
		: nls.localize('userIsSudo', '[Superuser]');
	private static readonly NLS_EXTENSION_HOST = nls.localize(
	...
	get minimumHeight(): number {
		return (
			30 / (this.currentMenubarVisibility === 'hidden' ? getZoomFactor() : 1)
		);
	}
	...
	constructor(
		...
		this.contextMenu = this._register(
			menuService.createMenu(MenuId.TitleBarContext, contextKeyService)
		);
	...
	protected onConfigurationChanged(event: IConfigurationChangeEvent): void {
		if (
			event.affectsConfiguration('window.title') ||
			event.affectsConfiguration('window.titleSeparator')
		) {
			this.titleUpdater.schedule();
		}
	...
	private onActiveEditorChange(): void {
		...
		if (activeEditor) {
			this.activeEditorListeners.add(
				activeEditor.onDidChangeDirty(() => this.titleUpdater.schedule())
			);
			this.activeEditorListeners.add(
	...
	private getWindowTitle(): string {
		...
		if (this.properties.isAdmin) {
			title = `${title || this.productService.nameLong} ${
				TitlebarPart.NLS_USER_IS_ADMIN
			}`;
		}

		if (!this.properties.isPure) {
			title = `${title || this.productService.nameLong} ${
				TitlebarPart.NLS_UNSUPPORTED
			}`;
		}
	...
	private doGetWindowTitle(): string {
		...
		const dirty =
			editor?.isDirty() && !editor.isSaving() ? TitlebarPart.TITLE_DIRTY : '';
		const appName = this.productService.nameLong;
		const remoteName = this.labelService.getHostLabel(
	...
	createContentArea(parent: HTMLElement): HTMLElement {
		...
		[EventType.CONTEXT_MENU, EventType.MOUSE_DOWN].forEach((event) => {
			this._register(
				addDisposableListener(this.title, event, (e) => {
			...
			addDisposableListener(
				...
				(e) => {
					if (
						e.target &&
						this.menubar &&
						isAncestor(e.target as HTMLElement, this.menubar)
					) {
						return;
					}
	...
	protected adjustTitleMarginToCenter(): void {
		if (this.customMenubar && this.menubar) {
			...
			if (
				leftMarker > (this.element.clientWidth - this.title.clientWidth) / 2 ||
