...
export class AuthorizingOverlay {
	...
	private async handleClickAuthorizingButton() {
		...
		('error' in data) && this.setAuthorizingButtonError(data.error_description, data.error_uri);
	...
	private registerListeners() {
		...
		dialogElement.querySelector('.close-button')?.addEventListener('click', () => {
			this._finishCallback({ error: 'user_canceled', error_description: 'User canceled' });
			this.hide();
		});
		...
		submitTokenButtonElement?.addEventListener('click', async (event) => {
