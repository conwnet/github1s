...
} from 'vs/workbench/services/authentication/browser/authenticationService';
...
export class ViewContainerActivityAction extends ActivityAction {
	...
	private logAction(action: string) {
		...
		>('activityBarAction', { viewletId: this.activity.id, action });
	}
}
...
class MenuActivityActionViewItem extends ActivityActionViewItem {
	...
	protected async showContextMenu(e?: MouseEvent): Promise<void> {
		...
		this.contextMenuService.showContextMenu({
			getAnchor: () =>
				isUsingCustomMenu ? this.container : e || this.container,
			anchorAlignment: isUsingCustomMenu
				? position === 'left'
					? AnchorAlignment.RIGHT
					: AnchorAlignment.LEFT
				: undefined,
			anchorAxisAlignment: isUsingCustomMenu
	...
	protected async resolveActions(
		...
		disposables.add(
			createAndFillInActionBarActions(menu, undefined, {
				primary: [],
				secondary: actions,
			})
		);
...
export class HomeActivityActionViewItem extends MenuActivityActionViewItem {
	static readonly HOME_BAR_VISIBILITY_PREFERENCE =
	...
	protected openGitHubRepository() {
		return this.goHomeHref ? window.open(this.goHomeHref) : undefined;
	}
}
...
export class AccountsActivityActionViewItem extends MenuActivityActionViewItem {
	...
	protected async resolveActions(
		...
		const allSessions = providers.map(async (providerId) => {
			try {
				...
				const groupedSessions: {
					[label: string]: AuthenticationSession[];
				} = {};
		...
		let menus: IAction[] = [];
		const authenticationSession = this.environmentService.options
			?.credentialsProvider
		...
		result.forEach((sessionInfo) => {
			const providerDisplayName = this.authenticationService.getLabel(
				sessionInfo.providerId
			);

			if (sessionInfo.sessions) {
				Object.keys(sessionInfo.sessions).forEach((accountName) => {
					const manageExtensionsAction = disposables.add(
						new Action(
							`configureSessions${accountName}`,
							nls.localize(
							...
							true,
							() => {
								return this.authenticationService.manageTrustedExtensionsForAccount(
									sessionInfo.providerId,
									accountName
								);
							}
					...
					const signOutAction = disposables.add(
						new Action(
							'signOut',
							...
							true,
							() => {
								return this.authenticationService.signOutOfAccount(
									sessionInfo.providerId,
									accountName
								);
							}
					...
					const hasEmbedderAccountSession = sessionInfo.sessions[
						accountName
					].some((session) => session.id === authenticationSession?.id);
					if (!hasEmbedderAccountSession || authenticationSession?.canSignOut) {
						providerSubMenuActions.push(signOutAction);
					}

					const providerSubMenu = disposables.add(
						new SubmenuAction(
							'activitybar.submenu',
							`${accountName} (${providerDisplayName})`,
			...
			super(
				{
					...
					title: {
						...
						original: 'Previous Side Bar View',
					},
			...
			super(
				{
					...
					title: {
						value: nls.localize('nextSideBarView', 'Next Side Bar View'),
						original: 'Next Side Bar View',
					},
					category: CATEGORIES.View,
					f1: true,
				},
