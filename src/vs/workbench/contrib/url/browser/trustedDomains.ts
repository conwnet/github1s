...
export async function configureOpenerTrustedDomainsHandler(
	...
	if (pickedResult && pickedResult.id) {
		telemetryService.publicLog2<
			{ choice: string },
			ConfigureTrustedDomainsChoiceClassification
			...
			case 'manage':
				...
				notificationService.prompt(
					...
					[
						{
							label: 'Copy',
							run: () => clipboardService.writeText(resource.toString()),
						},
					]
				);
...
export function extractGitHubRemotesFromGitConfig(gitConfig: string): string[] {
	...
	const RemoteMatcher = /^\s*url\s*=\s*(?:git@|https:\/\/)github\.com(?::|\/)(\S*)\s*$/gm;
...
export async function readTrustedDomains(
	...
	const [workspaceDomains, userDomains] = await Promise.all([
		readWorkspaceTrustedDomains(accessor),
		readAuthenticationTrustedDomains(accessor),
	]);
	return {
		workspaceDomains,
		userDomains,
